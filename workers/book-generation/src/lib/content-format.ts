...
export function resolveManuscriptLayout(
  ...
  if (
    PRACTICAL_GENRES.has(genre) ||
    STRUCTURED_TEMPLATES.has(norm(input.templateId))
  ) {
    return "practical";
  }
...
function buildFormatInstructions(profile: {
  ...
  if (profile.wantsFigures) {
    lines.push(
      ...
      "- Use figures for processes, maps, or conceptual diagrams — not decorative filler.",
  ...
  lines.push(
    ...
    "- Still write full paragraphs between structures."
  );
